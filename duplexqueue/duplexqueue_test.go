...
// out of range.
func insert(q *Duplexqueue, i int, elem interface{}) {
	if i < 0 || i > q.Len() {
		panic("duplexqueue: Insert() called with index out of range")
	}
	if i == 0 {
		q.PushFront(elem)
		return
...
func remove(q *Duplexqueue, i int) interface{} {
	if i < 0 || i >= q.Len() {
		panic("duplexqueue: Remove() called with index out of range")
	}
	if i == 0 {
		return q.PopFront()
	}
